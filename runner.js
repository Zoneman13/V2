...
sendButImage(anu.jid, teks, `©Created By ZONEMAN`, img,but = [{buttonId:`hallo`, 
...
img = await getBuffer(ppUrl)
teks = `Bye Bye @${num.split('@')[0]}\nJANGAN LUPA SENYUM: ${mdata.subject}`
sendButImage(anu.jid, teks, `©Created By ZONEMAN`, img,but= [{buttonId: `byebye`, buttonText: {displayText: `SELAMAT TINGGAL!!!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, number]}, thumbnail: Buffer.alloc(0)})
}
